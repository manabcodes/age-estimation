...
# Custom conservative transforms
class RandomNoise(object):
    def __init__(self, mean=0, std=0.002):  # Very small noise
        self.mean = mean
        self.std = std
...
class RandomGamma(object):
    def __init__(self, gamma_range=(0.95, 1.05)):  # Very subtle gamma changes
...
train_transform = transforms.Compose([
    ...
    transforms.RandomHorizontalFlip(),
    transforms.RandomRotation(degrees=3),  # Very small rotations
    transforms.RandomAffine(
        degrees=0, 
        translate=(0.05, 0.05),    # Very small translations (reduced from 0.1)
        scale=(0.98, 1.02),        # Very slight scale variations (reduced from 0.95-1.05)
        shear=1                    # Very minor shear (reduced from 2)
    ),
    RandomGamma(gamma_range=(0.95, 1.05)),  # Very subtle exposure changes
    transforms.ColorJitter(
        brightness=0.2, 
        contrast=0.2, 
        saturation=0.2,
        hue=0.05  # Small hue variation
    ),
    transforms.RandomGrayscale(p=0.05),  # Very rarely convert to grayscale (reduced from 0.1)
    transforms.RandomApply([
        transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 1.0))
    ], p=0.15),  # Simulate slight blur
    transforms.ToTensor(),
    RandomNoise(std=0.002),  # Very minimal noise
    transforms.RandomErasing(
        p=0.05,                    # Very low probability (reduced from 0.1)
        scale=(0.01, 0.05),        # Very small erasure areas (reduced from 0.02-0.1)
        ratio=(0.3, 3.3),          # Standard aspect ratio range
        value=0                    # Erase to black
    ),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
])
...
complexity_report_path = os.path.join(config['output'], 'sord-model_complexity.txt')
detailed_report_path = os.path.join(config['output'], 'sord-model_detailed_analysis.txt')
...
# Save final model
final_model_path = os.path.join(config['output'], "resnet50_sord_final.pth")
torch.save({
    'model_state_dict': model.state_dict(),
    'optimizer_state_dict': optimizer.state_dict(),
...
prof_report_path = os.path.join(config['output'], 'sord-profiler_results.txt')
...
plt.grid(True)
plt.savefig(os.path.join(config['output'], 'sord-learning_rate_schedule.png'))
plt.show()
